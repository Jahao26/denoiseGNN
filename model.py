...
class myModel(nn.Module):
    def __init__(self, args, num_class):
        ...
        self.graph_dim = args.hidden_dim * (args.gnn_layers + 1)
        ...
        self.lstm_f = MyLSTM(args.emb_dim, self.lstm_hidden_dim, self.graph_dim) 

        # output mlp layers
        layers = [nn.Linear(self.lstm_hidden_dim, args.hidden_dim), nn.PReLU()]
...
class ablationModel(nn.Module):
    def __init__(self, args, num_class):
        ...
        # output mlp layers
        layers = [nn.Linear(args.emb_dim + self.graph_dim, args.hidden_dim), nn.PReLU()]
        for _ in range(args.mlp_layers - 1):
            layers += [nn.Linear(args.hidden_dim, args.hidden_dim), nn.PReLU()]
        layers += [self.dropout]
        layers += [nn.Linear(args.hidden_dim, num_class)]
    ...
    def forward(self, features, s_mask, entro, alpha, threshold):
        ...
        '''
        num_utter = features.size()[1]
        adj = self.filter(features, entro, alpha, threshold) 
        H0 = self.activation(self.fc1(features))  
        H = [H0]
